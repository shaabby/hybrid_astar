# Hybrid A* 路径规划算法与可视化系统设计实现报告

## 摘要

本项目实现了一个面向车辆路径规划教学展示的 Hybrid A* 路径规划程序。程序以栅格地图、起点、终点和车辆参数为输入，使用带车辆运动学约束的 Hybrid A* 算法搜索从起点到终点的连续可行路径，并将规划结果导出为 JSON。项目使用 C++23 编写核心规划逻辑，并提供基于 FLTK 的独立路径 JSON 交互式查看工具，支持路径回放、逐帧步进、搜索树增量可视化和惩罚参数对比实验，便于课堂演示、结果复现和批量实验分析。
...
- `include/` 存放头文件和主要类定义，含 `FltkCanvas.hpp`、`FltkViewer.hpp` 等可视化模块。
- `src/` 存放 C++ 实现文件，含 `FltkCanvas.cpp`（画布渲染与离屏缓冲管理）、`FltkViewer.cpp`（播放控制与交互）。
...
### 3.5 独立路径 JSON 交互式查看工具

项目提供独立的 `tool/path_json_viewer.cpp` 和 `tool/view_path_json.sh`，基于 FLTK 实现完整的交互式路径回放界面，支持：

- **播放 / 暂停 / 步进 / 重置**：可逐帧播放规划路径，或手动拖拽时间轴滑块跳转。
- **搜索树增量可视化**：在规划树只增不减的假设下，随播放逐帧追加新出现的搜索树分支到离屏缓冲区，避免每帧全量遍历搜索树。
- **分层离屏渲染**：将静态地图（栅格、障碍物、起终点标记）与动态层（路径轨迹、搜索树、车辆）分离，`draw()` 只做离屏缓冲区 blit + 画车辆，大幅减少每帧 CPU 绘制调用。

查看器默认只加载最终路径，跳过大量扩展节点数据，保证大规模实验输出也能快速打开。
